...
def main(args):
    ...
    for (theme_id, theme_name) in themes:
        ...
        if _theme is None:
            ...
            query = 'select id, text, goodness, mf, stance from phrase where theme_id = {}'.format(theme_id)
            cur_ns.execute(query)
            phrases = cur_ns.fetchall()
            print("..Adding {} phrases".format(len(phrases)))

            # Add all pahrases into NN database
            for (phrase_id, text, goodness, mf, stance) in phrases:
                #print(phrase_id, goodness, mf, stance)
                if '"' in text:
                    text = text.replace('"', "'")
                query = '''insert into phrase (id, text, goodness, mf, stance, theme_id) values ({}, "{}", "{}", "{}", "{}", {})'''.format(phrase_id, text, goodness, mf, stance, theme_id)
                query = query.format(phrase_id, text, goodness, stance, mf, theme_id)
                cur_nn.execute(query)

            # Find all tweets from NS DB
            query = 'select id, good from tweet where theme_id = {}'.format(theme_id)
            cur_ns.execute(query)
            ...
            for (tw_id, good) in tweets:
                query = 'update tweet set good = {}, theme_id = {} where id = {}'.format(good, theme_id, tw_id)
                cur_nn.execute(query)
