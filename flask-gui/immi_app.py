...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///immi.db'
...
app.config['UPLOAD_FOLDER'] = 'static/immi'
# 500 MB
...
def assign_fixed_clusters():
    ...
    for i in range(0, tweet_embed_torch.shape[0]):
        tweet = TweetImmi.query.filter_by(tweet_id=str(i)).first()
        ...
        #print(themes[top_index].name, top_score, cos_scores)
...
def run_hdbscan():
    ...
    for j, i in zip(tweet_ids, range(0, len(tweet_ids))):
        ...
        if cluster_idx >= 0:
            ...
            tw = TweetImmi.query.filter_by(tweet_id=j).first()
...
def kmeans():
    ...
    for j, i in zip(tweet_ids, range(0, len(tweet_ids))):
        ...
        if tw is not None:
           ...
           # Create the tweet
            tw = TweetImmi(tweet_id=j, text=tweet_text[j], theme=theme, distance=distance_to_centroid)
            db.session.add(tw)
...
def wordclouds(theme_name):
    ...
    for theme in themes:
        tweets = TweetImmi.query.join(TweetImmi.theme, aliased=True).filter_by(name=theme.name).order_by('distance').limit(K_curr).all()
...
def narrative_distribution(theme, tweets, ts):
    narrative_img = 'narrative_{}_{}.png'.format(theme.name, ts)

    if len(tweets) > 0:
        narratives = [t.narrative for t in tweets]
        counts = [narratives.count('Episodic'), narratives.count('Thematic')]
        print(theme.name, counts)
        clrs = ['royalblue', 'crimson']
        f, ax = plt.subplots(figsize=(5,5)) # set the size that you'd like (width, height)
        plt.xticks(rotation=60)
        plt.bar(['Episodic', 'Thematic'], counts, color=clrs)
        #plt.bar(theme_names, counts, color=clrs, width=0.4)
        #ax.legend(fontsize = 10)
        plt.savefig(os.path.join(app.config['UPLOAD_FOLDER'], 'images', narrative_img), bbox_inches = "tight")
        #plt.show()
        plt.close()
    return narrative_img

def policy_frame_distribution(theme, tweets, ts):
    frame_img = 'policy_frame_{}_{}.png'.format(theme.name, ts)

    frames = []
    if len(tweets) > 0:
        for t in tweets:
            if t.frame_political:
            ...
            if t.frame_economic:
                frames.append('Economic')

        keys = list(set(frames))
        counts = [frames.count(k) for k in keys]
        f, ax = plt.subplots(figsize=(5,5))
        plt.xticks(rotation=60)
        my_cmap = plt.get_cmap("viridis")
        plt.bar(keys, counts, color=my_cmap(counts))
        plt.savefig(os.path.join(app.config['UPLOAD_FOLDER'], 'images', frame_img), bbox_inches = "tight")
        #plt.show()
        plt.close()
    return frame_img

def immi_frame_distribution(theme, tweets, ts):
    frame_img = 'immi_frame_{}_{}.png'.format(theme.name, ts)

    if len(tweets) > 0:
        frames = [t.immi_frame for t in tweets]
        frames = ['None' if t.immi_frame is None else t.immi_frame for t in tweets]
        keys = list(set(frames))
        counts = [frames.count(k) for k in keys]
        f, ax = plt.subplots(figsize=(5,5))
        plt.xticks(rotation=60)
...
def immi_role_distribution(theme, tweets, ts):
    role_img = 'immi_role_{}_{}.png'.format(theme.name, ts)

    if len(tweets) > 0:
        roles = [t.immi_role for t in tweets]
        roles = ['None' if t.immi_role is None else t.immi_role for t in tweets]
        keys = list(set(roles))
        counts = [roles.count(k) for k in keys]
        f, ax = plt.subplots(figsize=(5,5))
        plt.xticks(rotation=60)
        my_cmap = plt.get_cmap("viridis")
        plt.bar(keys, counts, color=my_cmap(counts))
        plt.savefig(os.path.join(app.config['UPLOAD_FOLDER'], 'images', role_img), bbox_inches = "tight")
        #plt.show()
        plt.close()
    return role_img
...
def symbol_distribution(theme_name):
    os.system('rm {}/narrative*'.format(os.path.join(app.config['UPLOAD_FOLDER'], 'images')))
    os.system('rm {}/policy_frame*'.format(os.path.join(app.config['UPLOAD_FOLDER'], 'images')))
    os.system('rm {}/immi_frame*'.format(os.path.join(app.config['UPLOAD_FOLDER'], 'images')))

    theme = Theme.query.filter_by(name=theme_name).first()
    tweets = TweetImmi.query.join(TweetImmi.theme, aliased=True).filter_by(name=theme.name).order_by('distance').all()

    ts = time.time()
    narrative_img = os.path.join('immi', 'images', narrative_distribution(theme, tweets, ts))
    policy_frame_img = os.path.join('immi', 'images',policy_frame_distribution(theme, tweets, ts))
    immi_frame_img = os.path.join('immi', 'images',immi_frame_distribution(theme, tweets, ts))
    role_img = os.path.join('immi', 'images',immi_role_distribution(theme, tweets, ts))

    return narrative_img, policy_frame_img, immi_frame_img, role_img

def update_theme_centroids(theme_name):
    global theme_centroids
    print(theme_name)
    # Add the good phrases
    phrases = PhraseImmi.query.filter_by(goodness='good').join(PhraseImmi.theme, aliased=True).filter_by(name=theme_name).all()
    ...
    if len(phrases) > 0:
       ...
       tweets = TweetImmi.query.join(TweetImmi.theme, aliased=True).filter_by(name=theme_name).all()
...
def mark_as_good():
    for checkbox in request.form.getlist('checkbox'):
        tweet = TweetImmi.query.get(checkbox)
        tweet.good = True
        phrase = PhraseImmi.query.filter_by(text=tweet.text).first()
        if not phrase:
            new_phrase = PhraseImmi(text=tweet.text, theme=tweet.theme, goodness='good',
                            immi_frame=tweet.immi_frame,
                            narrative=tweet.narrative,
                            immi_role=tweet.immi_role,
                            frame_political=tweet.frame_political,
                            frame_policy=tweet.frame_policy,
                            frame_crime=tweet.frame_crime,
                            frame_health=tweet.frame_health,
                            frame_security=tweet.frame_security,
                            frame_economic=tweet.frame_economic)
...
def assign_to_theme(tweet_ids, theme_name):
    for tweet_id in tweet_ids:
        tweet = TweetImmi.query.get(tweet_id)
        theme = Theme.query.filter_by(name=theme_name).first()
        tweet.theme = theme
        tweet.good = True
...
def data():
    ...
    if form_explore.validate_on_submit():
        if 'close' in request.form:
            tweets = TweetImmi.query.join(TweetImmi.theme, aliased=True).filter_by(name=form_explore.theme.data).order_by('distance').limit(100).all()
        else:
            tweets = TweetImmi.query.join(TweetImmi.theme, aliased=True).filter_by(name=form_explore.theme.data).order_by(desc(TweetImmi.distance)).limit(100).all()

        if len(tweets) > 0:
            empty_table = False
            first = TweetImmi.query.first()
    ...
    if form_search.validate_on_submit():
        ...
        tweets = [TweetImmi.query.filter_by(tweet_id=id).one() for id in top_results]
...
def delete_theme(theme_name):
    ...
    os.remove('{}/narrative_{}.png'.format(os.path.join(app.config['UPLOAD_FOLDER'], 'images'), theme_name))
    os.remove('{}/policy_frame_{}.png'.format(os.path.join(app.config['UPLOAD_FOLDER'], 'images'), theme_name))
    os.remove('{}/immi_frame_{}.png'.format(os.path.join(app.config['UPLOAD_FOLDER'], 'images'), theme_name))
...
def edit_theme(theme_name, new_name):
    ...
    os.rename('{}/policy_frame_{}.png'.format(os.path.join(app.config['UPLOAD_FOLDER'], 'images'), theme_name),
              '{}/policy_frame_{}.png'.format(os.path.join(app.config['UPLOAD_FOLDER'], 'images'), new_name))

    os.rename('{}/immi_frame_{}.png'.format(os.path.join(app.config['UPLOAD_FOLDER'], 'images'), theme_name),
              '{}/immi_frame_{}.png'.format(os.path.join(app.config['UPLOAD_FOLDER'], 'images'), new_name))

    os.rename('{}/narrative_{}.png'.format(os.path.join(app.config['UPLOAD_FOLDER'], 'images'), theme_name),
              '{}/narrative_{}.png'.format(os.path.join(app.config['UPLOAD_FOLDER'], 'images'), new_name))
...
@app.route('/themesimmi', methods=['GET', 'POST'])
def themes():
    form_explore = ThemeForm()
    form_explore.theme.choices = [t.name for t in Theme.query.order_by('name').all()]

    form_new_code = NewCodeForm()
    form_new_phrase = NewPhraseImmiForm()

    theme = None;
    word_cloud_img = None; 
    narrative_img=None; policy_frame_img=None; immi_frame_img = None; role_img = None
    exists_file = False
    ...
    if request.method == 'POST':
        if form_explore.validate_on_submit() and 'visualize' in request.form:
            ...
            narrative_img, policy_frame_img, immi_frame_img, role_img = symbol_distribution(theme_name)
            exists_file = os.path.exists(os.path.join(app.config['UPLOAD_FOLDER'], 'images', word_cloud_img))
            print("Exists file", exists_file)
            word_cloud_img = os.path.join('immi', 'images', wordclouds(theme_name))

            theme = Theme.query.filter_by(name=theme_name).first()
            good_phrases = PhraseImmi.query.filter_by(goodness='good').join(PhraseImmi.theme, aliased=True).filter_by(name=theme_name).all()
            bad_phrases = PhraseImmi.query.filter_by(goodness='bad').join(PhraseImmi.theme, aliased=True).filter_by(name=theme_name).all()
            print(good_phrases)
    ...
    if form_new_phrase.validate_on_submit() and 'theme_name_f2' in request.form:
        ...
        theme = Theme.query.filter_by(name=theme_name).first()
        phrase = PhraseImmi(text=form_new_phrase.text.data, theme=theme,
                        goodness=form_new_phrase.goodness.data,
                        narrative=form_new_phrase.narrative.data,
                        immi_frame=form_new_phrase.immi_frame.data,
                        immi_role=form_new_phrase.immi_role.data,
                        frame_political=form_new_phrase.frame_political.data,
                        frame_policy=form_new_phrase.frame_policy.data,
                        frame_crime=form_new_phrase.frame_crime.data,
                        frame_health=form_new_phrase.frame_health.data,
                        frame_security=form_new_phrase.frame_security.data,
                        frame_economic=form_new_phrase.frame_economic.data)
        update_theme_centroids(theme_name)
    ...
    return render_template("themesimmi.html", form_explore=form_explore, theme=theme, word_cloud_img=word_cloud_img,
                           narrative_img=narrative_img, policy_frame_img=policy_frame_img, immi_frame_img=immi_frame_img,
                           role_img=role_img, form_new_code=form_new_code, exists_file=exists_file,
...
def plot_tsne(ts):
    ...
    for i, theme in enumerate(themes):
        ...
        tweets = TweetImmi.query.join(TweetImmi.theme, aliased=True).filter_by(name=theme.name).order_by('distance').limit(500).all()
...
def coverage():
    ...
    theme_names = []; counts = []
    for theme in themes:
        num_tweets = TweetImmi.query.join(TweetImmi.theme, aliased=True).filter_by(name=theme.name).count()
        if num_tweets > 0:
    ...
    # Fix the path
    bar_plot_img = os.path.join('immi', 'images', bar_plot_img)
    pie_plot_img = os.path.join('immi', 'images', pie_plot_img)
    tsne_img = os.path.join('immi', 'images', tsne_img)
...
def similar(tweet_ids):
    ...
    query_tweets = TweetImmi.query.filter(TweetImmi.id.in_(tweet_ids)).all()
...
def delete_phrase(phrase_id):
    phrase = PhraseImmi.query.get(phrase_id)
    db.session.delete(phrase)
    db.session.commit()
    if phrase:
        return f'Deleting {phrase_id}. Return to <a href="/themesimmi">themes</a>.'
    return f'Could not delete message {phrase_id} as it does not exist. Return to <a href="/themesimmi">themes</a>'

@app.route('/edit_phrase/<phrase_id>/edit', methods=['GET', 'POST'])
def edit_phrase(phrase_id):
    phrase = PhraseImmi.query.get(phrase_id)
    form_edit_phrase = EditPhraseImmiForm()

    if form_edit_phrase.validate_on_submit():
        phrase.goodness=form_edit_phrase.goodness.data
        phrase.narrative=form_edit_phrase.narrative.data
        phrase.immi_frame=form_edit_phrase.immi_frame.data
        phrase.immi_role=form_edit_phrase.immi_role.data

        phrase.frame_political=form_edit_phrase.frame_political.data
        phrase.frame_policy=form_edit_phrase.frame_policy.data
        phrase.frame_crime=form_edit_phrase.frame_crime.data
        phrase.frame_health=form_edit_phrase.frame_health.data
        phrase.frame_security=form_edit_phrase.frame_security.data
        phrase.frame_economic=form_edit_phrase.frame_economic.data

        db.session.commit()
        return f'Edited {phrase_id}. Return to <a href="/themesimmi">themes</a>.'
...
def create_tables():
    ...
    for theme in themes:
        ...
        if num_tweets == 0:
            ...
            phrases = PhraseImmi.query.filter_by(theme_id=theme.id).all()
